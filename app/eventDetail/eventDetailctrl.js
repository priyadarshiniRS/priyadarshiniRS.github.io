export default class eventDetailctrl{
  ...
  constructor($scope , $window , $rootScope , $location,$http, detailfact,$stateParams, $localStorage,$state,$timeout){
    ...
    this.detailfact.getEventDetail(this.sp.id).success((res)=>{
      ...
      var test=moment(String(res.event[0].startsOn.date).split(' ')[0]).format('dddd,Do MMM, YYYY');
      this.timeout(()=>{
        this.scope.date={
          eventDate:test
        };
...
eventdata(){
  ...
  var dayConvert={Mon:"Monday",Tue:"Tuesday",Wed:"Wednesday",Thu:"Thursday",Fri:"Friday",Sat:"Saturday",Sun:"Sunday"}
