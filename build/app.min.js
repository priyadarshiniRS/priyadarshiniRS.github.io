...
var businesscardCtrl = function () {
  ...
  function businesscardCtrl($scope, $location, $http, detailfact, $stateParams, $localStorage, $window, $timeout) {
    ...
    this.timeout(function () {
      console.log("ajay", _this.ls);
      _this.scope.person = _this.ls.person[0];
      _this.scope.editperson = angular.copy(_this.ls.person[0]);
...
var eventlistctrl = function () {
  ...
  function eventlistctrl($scope, $location, detailfact, $localStorage, $timeout, $state) {
    ...
    detailfact.getDetails().success(function (res) {
      ...
      for (var _i = 1; _i <= res.length - 1; _i++) {
        if (dummyDate < todayDate) {
          dummyDate = Date.parse(String(moment(res[_i].startsOn.date).format().split('T')[0]));
          console.log(_i, "if", dummyDate);
          continue;
        } else {
          dummyDate = moment(dummyDate).format().split('T')[0];
          console.log(_i, "else", new Date(moment(dummyDate).format().split('T')[0]));
          ...
          _this.timeout(function () {
            ...
            for (var _i2 = 0; _i2 < 7; _i2++) {
              _this.indexArr.push(dummyDate);
              _this.dateObj.date = dummyDate;
              _this.dateObj.printDate = dummyDate.split('-')[2];
              var tempMonth = String(moment(dummyDate).format('LL')).split(' ');
              _this.dateObj.month = tempMonth[0] + " " + tempMonth[2];
              _this.dateObj.day = moment(dummyDate).format('LLLL').split(',')[0];
              _this.dateArr.push(_this.dateObj);
              _this.dateObj = {};
              dummyDate = moment(dummyDate).add(1, 'days').format().split('T')[0];
            }
            _this.timeout(function () {
              _this.scope.final = _this.dateArr;
              _this.scope.currentTab = _this.indexArr.indexOf(showDate);
            }, 0);
          }, 0);
          break;
        }
  ...
  _createClass(eventlistctrl, [{
    ...
    value: function getevents(date) {
      ...
      this.timeout(function () {
        _this2.scope.slickConfig2.method.slickGoTo(_this2.scope.currentTab);
      }, 0);
    ...
    key: "calEvent",
    value: function calEvent(dummyDate) {
      var _this4 = this;
      ...
      this.timeout(function () {
        ...
        _this4.dateObj = {};
        for (var i = 0; i < 7; i++) {
          _this4.indexArr.push(dummyDate);
          _this4.dateObj.date = dummyDate;
          _this4.dateObj.printDate = dummyDate.split('-')[2];
          var tempMonth = String(moment(dummyDate).format('LL')).split(' ');
          _this4.dateObj.month = tempMonth[0] + " " + tempMonth[2];
          _this4.dateObj.day = moment(dummyDate).format('LLLL').split(',')[0];
          _this4.dateArr.push(_this4.dateObj);
          _this4.dateObj = {};
          dummyDate = moment(dummyDate).add(1, 'days').format().split('T')[0];
        }
        _this4.timeout(function () {
          _this4.scope.final = _this4.dateArr;
          _this4.scope.currentTab = _this4.indexArr.indexOf(showDate);
          _this4.getevents(showDate);
        }, 0);
      }, 0);
    }
  }, {
