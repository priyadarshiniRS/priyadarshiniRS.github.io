...
var businesscardCtrl = function () {
  /*@ngInject;*/
  businesscardCtrl.$inject = ["$scope", "$location", "$http", "detailfact", "$stateParams", "$localStorage", "$window"];
  function businesscardCtrl($scope, $location, $http, detailfact, $stateParams, $localStorage, $window) {
...
var footerCtrl =
/*@ngInject;*/
["detailfact", "$scope", "$window", "$timeout", "$interval", function footerCtrl(detailfact, $scope, $window, $timeout, $interval) {
...
var eventlistctrl = function () {
  ...
  function eventlistctrl($scope, $location, detailfact, $localStorage, $timeout) {
    ...
    detailfact.getDetails().success(function (res) {
      ...
      for (var i = 1, j = 0; i <= 30; i++, j++, dayIndex++) {
        if (i < 10) {
          i = "0" + String(i);
        }
        dayIndex = dayIndex % 7;
        ...
        _this.tempobj.printDate = String(i);
      ...
      firstDate = String(firstDate).split(' ');
      var dayIndex = day.indexOf(firstDate[0]);
      for (var _i = 1, _j = 0; _i <= 31; _i++, _j++, dayIndex++) {
        if (_i < 10) {
          _i = "0" + String(_i);
        }
        dayIndex = dayIndex % 7;
        _this.tempobj1.day = _this.days[dayIndex];
        d = moment('2016-12-' + _i).format().split('T')[0];
        _this.tempobj1.date = d;
        tempMonth = String(moment(d).format('LL')).split(' ');
        _this.tempobj1.month = tempMonth[0] + ' ' + tempMonth[2];
        _this.tempobj1.printDate = String(_i);
      ...
      for (var _i2 = 1, _j2 = 0; _i2 <= 31; _i2++, _j2++, dayIndex++) {
        ...
        d = moment('2017-01-' + _i2).format().split('T')[0];
        _this.tempobj2.date = d;
        tempMonth = String(moment(d).format('LL')).split(' ');
        _this.tempobj2.month = tempMonth[0] + ' ' + tempMonth[2];
        _this.tempobj2.printDate = String(_i2);
      ...
      var _loop = function _loop(_i4) {
        _this.detailfact.getDateDetail(_this.indexArray[_i4]).success(function (res) {
          if (res.code == 3 || res.code == 4) {
            _i4--;
            // console.log(i);
          } else {
            // console.log(i,this.indexArray[i],"i ,this.indexArray[i]");
            _this.scope.currentTab = _i4;
          }
        });
        _i3 = _i4;
      };

      for (var _i3 = _this.indexArray.length - 1; Date.parse(_this.indexArray[_i3]) > todayDate; _i3--) {
        _loop(_i3);
      }
